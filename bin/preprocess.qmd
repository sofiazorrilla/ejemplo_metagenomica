...
data.frame(n = sample_sums(metagenomes)) %>% ggplot(aes(x = rownames(.), y = n))+geom_col()
```
...
metadata = read.csv("../data/Camel_milk_Microbiome_analysis/metadata.csv", header = T) %>% select(Sample.Name, Lat_Lon, Group)
